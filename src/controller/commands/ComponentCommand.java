...
/**
 * Command to extract a specific component from an image, with optional split view or masking.
 */
public class ComponentCommand implements Command {
  ...
  /**
   * Constructs a component extraction command with the given parameters.
   *
   ...
   */
  public ComponentCommand(String[] tokens, AdvancedImageProcessor imageProcessor,
                          ImageModelInterface model) {

    if (tokens.length < 3 || tokens.length > 5) {
      throw new IllegalArgumentException("Usage: " +
              "[component]-component sourceImage [maskImage] destImage [split p]");
    }

    this.componentName = tokens[0].split("-")[0].toLowerCase();
  ...
  private void parseArguments(String[] tokens) {
    ...
    if (tokens.length == 5) {
      if (tokens[3].equalsIgnoreCase("split")) {
        ...
        throw new IllegalArgumentException("Invalid syntax " +
                "or cannot use both mask image and split view.");
      }
      return;
    }
  ...
  /**
   * Executes the component extraction command on the given image.
   */
  @Override
  public void execute() {
    ...
    if (maskImageName != null) {
      ...
      result = imageProcessor.applyMask(image, maskImage, componentName + "-component");
    } else {
      result = imageProcessor.extractComponent(image, componentName);
    }
